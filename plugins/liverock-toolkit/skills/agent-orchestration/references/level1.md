...
- 형식: 레벨 2 실행 기록과 같은 mermaid 블록 + 노드 표다. 노드 ID는 9절 「설계」의 부여
  규칙을 따른다 — 워커는 `N`, 제어는 `G`이고 접두별로 독립 번호를 매기며 한 번 준 ID는
  바꾸지 않는다. 다만 레벨 1 노드는 서로 의존하지 않으므로 mermaid에서 노드 사이에
  화살표(`-->`)를 그리지 않고 각 노드를 따로 나열한다. 견본은 아래 「실행 기록 서식」에
  있다. 표의 열 집합은 레벨 2 표와 같게 두되 `노드 ID`·`프로필`·`상태`·`agentId`를 반드시
  포함한다. 각 행의 `agentId`는 그 노드의 `create_agent` 호출 결과를 받는 즉시 채운다 —
  아직 기동하지 않은 노드의 칸은 비워 둔다.
- **상태 값** — `대기` / `실행 중` / `완료` / `실패` / `생략`. 이 다섯 개만 쓰고 새 값을
  만들지 않는다. `생략`은 판정이 생략인 단계로 워커를 만들지 않으며 `create_agent` 대상이
  아니다. `완료` → `실행 중` 역전이는 아래 「재작업 표기」의 경우에만 일어나는 정상 전이다.
- **버전 줄** — `GRAPH.md` **맨 위, mermaid 블록보다 위**에 그 `GRAPH.md`를 만든 규칙의
  플러그인 버전을 한 줄로 적는다. 값은 플러그인 루트의 `plugin.json` `version` 필드에서 읽고,
  경로는 스킬 디렉터리(`skills/agent-orchestration/`) 기준 상대 경로
  `../../.codex-plugin/plugin.json` 하나다. 표기는 아래 「실행 기록 서식」 견본이 고정한다.
  값을 읽지 못하면 그 사실을 그 줄에 적고 실행을 멈추지 않으며 값을 추측하지 않는다. 이 줄은
  `GRAPH.md`를 만들 때만 쓰고, 이미 만들어져 버전 줄이 없는 기존 파일에는 소급하지 않는다 —
  고치라는 지시가 아니다.
...
- 기록 파일을 쓸 것과 그 경로는 승인 요청이 아니라 **기동 보고**(8절)에서 알린다.

## 실행 기록 서식

`GRAPH.md`에 두는 mermaid와 노드 표다. 맨 위 `버전 : ` 줄은 위 규칙이 정한 자리이고, 아래
견본이 그 표기(라벨 `버전`, 구분자 ` : `)를 고정한다.
...
확인값(`currentModeId`)이고, 모드가 없는 프로바이더는 `없음`으로 쓴다. mermaid 라벨에는 모델명만,
